...
if(isset($_GET['search'])){
	...
	// Memanggil method searchTransaksi untuk mencari data transaksi berdasarkan kata kunci dan menyimpan hasil dalam variabel $cariTransaksi
	$cariTransaksi = $transaksi->searchTransaksi($kataKunci);
} 
?>
<!doctype html>
<html lang="en">
							...
							<div class="col-12">
								<div class="card mb-3">
									...
									<div class="card-body">
										<form action="data-search.php" method="GET">
											<div class="mb-3">
												<label for="search" class="form-label">Masukkan Nama Customer</label>
												<input type="text" class="form-control" id="search" name="search" placeholder="Cari berdasarkan Nama Customer" value="<?php echo $kataKunci; ?>" required>
											</div>
								...
								<div class="card">
									...
									<div class="card-body">
										...
										// Mengecek apakah parameter GET 'search' ada
										if(isset($_GET['search'])){
											// Mengecek apakah ada data transaksi yang ditemukan
											if(count($cariTransaksi) > 0){
												...
												echo '<table class="table table-striped" role="table">
													...
													<tbody>';
													// Iterasi data transaksi yang ditemukan dan menampilkannya dalam tabel
													foreach ($cariTransaksi as $index => $trx){
														// Mengubah status transaksi menjadi badge dengan warna yang sesuai
														if($trx['status'] == 1){
															$trx['status'] = '<span class="badge bg-warning text-dark">Pending</span>';
														} elseif($trx['status'] == 2){
															$trx['status'] = '<span class="badge bg-success">Selesai</span>';
														} elseif($trx['status'] == 3){
															$trx['status'] = '<span class="badge bg-danger">Dibatalkan</span>';
														} 
														// Menampilkan baris data transaksi dalam tabel
												...
												// Menampilkan pesan jika tidak ada data transaksi yang ditemukan
